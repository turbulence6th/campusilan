...
(function(imageupload) {
	"use strict";
	imageupload.fn.filer = function(q) {
		return this.each(function(t, r) {
			var s = imageupload(r),
				...
				l = imageupload(),
				sl = [],
				n_f = imageupload.isFunction(q) ? q(s, imageupload.fn.filer.defaults) : q,
				n = n_f && imageupload.isPlainObject(n_f) ? imageupload.extend(true, {}, imageupload.fn.filer.defaults, n_f) : imageupload.fn.filer.defaults,
				f = {
					init: function() {
					...
					_changeInput: function() {
						...
						if(s.get(0)
							...
							.type != "file") {
							o = s;
							s = imageupload("<input type=\"file\" name=\"" + n.name + "\" />");
							s.css({
								position: "absolute",
								left: "-9999px",
								top: "-9999px",
							...
							if(n.changeInput) {
								switch(typeof n.changeInput) {
									case "boolean":
										o = imageupload('<div class="jFiler-input"><div class="jFiler-input-caption"><span>' + n.captions.feedback + '</span></div><div class="jFiler-input-button">' + n.captions.button + '</div></div>"');
										break;
									case "string":
									case "object":
										o = imageupload(n.changeInput);
										break;
									case "function":
										o = imageupload(n.changeInput(p, s, n));
										break;
								}
								s.after(o);
								s.css({
					...
					_reset: function(a) {
						...
						s.prop("jFiler").listEl = l = imageupload();
					},
					...
					_filesCheck: function() {
						...
						for(var t = 0; t < f.files.length; t++) {
							...
							if(n.extensions != null && imageupload.inArray(x, n.extensions) == -1) {
					...
					_thumbCreator: {
						create: function(i) {
							var file = f.files[i],
								...
								html = "";
							if(file.opts) {
								opts = imageupload.extend({}, file.opts, opts);
							}
							html = imageupload(f._thumbCreator.renderContent(opts))
						...
						generateIcon: function(obj) {
							...
							if(m[2] == 1) {
								...
								if(c) {
									var j = imageupload(el)
										.appendTo("body"),
										h = j.css("box-shadow");
									h = c + h.substring(h.replace(/^.*(rgba?\([^)]+\)).*imageupload/, 'imageupload1')
										.length, h.length);
									j.css({
											'-webkit-box-shadow': h,
											'-moz-box-shadow': h,
						...
						_box: function(params) {
							...
							if(l.length < 1) {
								if(n.appendTo) {
									var appendTo = imageupload(n.appendTo);
								} else {
									var appendTo = p;
								}
								appendTo.find('.jFiler-items')
									.remove();
								l = imageupload('<div class="jFiler-items jFiler-row"></div>');
								...
								l.on('click', n.templates._selectors.remove, function(e) {
									...
									if(cf) {
										f._remove(params ? params.remove.event : e, params ? params.remove.el : imageupload(this)
					...
					_upload: function(i) {
						...
						if(n.uploadFile.data != null && imageupload.isPlainObject(n.uploadFile.data)) {
					...
					_ajax: {
						send: function(el, formData, c) {
							c.ajax = imageupload.ajax({
								url: n.uploadFile.url,
								data: formData,
								type: n.uploadFile.type,
								enctype: n.uploadFile.enctype,
								xhr: function() {
									var myXhr = imageupload.ajaxSettings.xhr();
									if(myXhr.upload) {
					...
					_dragDrop: {
						...
						_dragLeaveCheck: function(e) {
							...
							if(related !== o) {
								if(related) {
									inside = imageupload.contains(o, related);
								}
								if(inside) {
									return false;
								}
					...
					_onSelect: function(i) {
						if(n.uploadFile && !imageupload.isEmptyObject(n.uploadFile)) {
							f._upload(i)
						}
					...
					_onChange: function(e, d) {
						...
						if(!n.uploadFile && n.addMore) {
							var elem = imageupload('<input type="file" />');
							var attributes = s.prop("attributes");
							imageupload.each(attributes, function() {
					...
					_retryUpload: function(e, data) {
						...
						if(obj.length > 0) {
							if(n.uploadFile && !imageupload.isEmptyObject(n.uploadFile) && !obj[0].uploaded) {
								f._itFc = obj[0];
								s.prop("jFiler")
									.current_file = f._itFc;
								f._upload(id);
						...
						var attrId = el.get(0)
							...
							excl_input = function(id) {
								var input = p.find("input[name^='jfiler-items-exclude-']:hidden")
									...
									val = [];
								if(input.size() == 0) {
									input = imageupload('<input type="hidden" name="jfiler-items-exclude-' + (n.excludeName ? n.excludeName : (s.attr("name")
					...
					_assets: {
						...
						getIcon: function(ext, type) {
							...
							if(imageupload.inArray(type, types) > -1) {
								return '<i class="icon-jfi-file-' + type + ' jfi-file-ext-' + ext + '"></i>';
							}
							return '<i class="icon-jfi-file-o jfi-file-type-' + type + ' jfi-file-ext-' + ext + '"></i>';
						},
						textParse: function(text, opts) {
							opts = imageupload.extend({}, {
								limit: n.limit,
								maxSize: n.maxSize,
								extensions: n.extensions ? n.extensions.join(',') : null,
							}, (opts && imageupload.isPlainObject(opts) ? opts : {}), n.options);
							switch(typeof(text)) {
								case "string":
